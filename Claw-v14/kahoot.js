...
(function () {
  ...
  /* ================================================================
     GROQ — prompt universal para todas as matérias
  ================================================================ */
  async function askGroq(question, options) {
    const system = `Você é um assistente de alta precisão para responder questões escolares e universitárias.
Domínios que você domina com excelência:
- MATEMÁTICA: álgebra, geometria, trigonometria, cálculo, equações, porcentagem, frações, estatística, probabilidade, funções, matrizes, logaritmos, progressões
- FÍSICA: mecânica, cinemática, dinâmica, energia, eletricidade, magnetismo, óptica, termodinâmica, ondas, leis de Newton
- QUÍMICA: tabela periódica, ligações, reações, estequiometria, soluções, ácidos/bases, orgânica, inorgânica, mol, balanceamento
- BIOLOGIA: célula, genética, evolução, ecologia, anatomia, fisiologia, botânica, zoologia, DNA/RNA, metabolismo
- HISTÓRIA: Brasil, mundo, datas, personagens, revoluções, guerras, períodos históricos, causas e consequências
- GEOGRAFIA: países, capitais, relevo, clima, biomas, cartografia, geopolítica, hidrografia, fusos horários
- PORTUGUÊS: gramática, ortografia, interpretação de texto, literatura, figuras de linguagem, análise sintática, morfologia, redação
- INGLÊS: vocabulário, gramática, verbos, tempos verbais, interpretação
- FILOSOFIA e SOCIOLOGIA: pensadores, conceitos, correntes filosóficas, sociedade
- ATUALIDADES e CULTURA GERAL

REGRA ABSOLUTA: Responda APENAS com o número da alternativa correta (1, 2, 3 ou 4). Nenhuma palavra além do número.
Pense passo a passo internamente antes de responder, mas mostre SOMENTE o número final.`;

    const user = `Questão: "${question}"
...
Qual o número da alternativa correta?`;
    ...
    // fallback: compara texto das opções
    const rLow = reply.toLowerCase();
    for (let i = 0; i < options.length; i++) {
      const oLow = options[i].toLowerCase();
      if (rLow.includes(oLow.slice(0, 15)) || oLow.includes(rLow.slice(0, 15))) return i;
    }
  ...
  /* ================================================================
     GROQ PARA DISSERTATIVAS — resolve e responde em texto curto
  ================================================================ */
  async function askGroqText(question) {
    const system = `Você é um especialista em todas as matérias escolares. Responda questões dissertativas de forma MUITO curta e direta (máximo 6 palavras). Sem explicações, sem pontuação extra. Apenas a resposta objetiva.
Exemplos:
