...
const MODEL = 'llama-3.3-70b-versatile'; // Llama 3.3 70B: estável e preciso
...
async function run(s){
  ...
  if(opts.length>=2){
    setUI('thinking','Consultando Llama 3.3...');
    startBar();
    try{
...
Responda SOMENTE com o número da alternativa correta (1, 2, 3 ou 4). Nenhuma palavra além do número. Não use tags de pensamento. Não explique.`;
...
async function askGroq(question,options){
  ...
  const txt = await new Promise((resolve, reject) => {
    chrome.runtime.sendMessage({
      ...
      ],max_tokens:256,temperature:0.3}  // R1 usa tokens internos para raciocinar
    }, res => {
  ...
  // Remove tags <think>...</think> do DeepSeek R1
...
<div id="__ck_card__">
  <div id="__ck_hd__">
    ...
    <span style="font-size:9px;color:#6b6b6b;margin-left:2px;">(Llama 3.3)</span>
