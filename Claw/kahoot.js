/* ================================================================
   CLAW — kahoot.js  (FINAL)
   Modelo: llama-3.1-8b-instant  (rápido + preciso)
   Detecção em 4 camadas. Um único processo. Sem dupla verificação.
...
(function () {
  ...
  let ON=false, STEALTH=false, XDELAY=0, busy=false, lastQ='', tickId=null, ui=null, barT=null;
  ...
  function snap() {
    ...
    const Q_SELS = [
      ...
      // camada 3: role semântico
      '[role="heading"]',
      // camada 4: tags comuns com texto longo
      'h1','h2','h3',
    ];
    let question = '';
    for (const s of Q_SELS) {
      try {
        const els = document.querySelectorAll(s);
        for (const el of els) {
          const t = (el.innerText||'').trim();
          if (t.length > 3 && t.length < 600 && vis(el)) {
            question = t; break;
          }
        }
        if (question) break;
    ...
    const A_SELS = [
      ...
      '[class*="AnswerLabel"]', '[class*="answerLabel"]',
      '[class*="choice-text"]', '[class*="option-text"]',
    ];
    let ansEls=[], options=[];
    for (const s of A_SELS) {
      try {
        const found = [...document.querySelectorAll(s)].filter(vis);
        if (found.length >= 2) {
          ansEls  = found;
          options = found.map(e=>(e.innerText||'').trim()).filter(Boolean);
          break;
        }
      } catch {}
    }
    ...
    /* fallback: botões coloridos do Kahoot (divs grandes com texto) */
    if (ansEls.length < 2) {
      try {
        const candidates = [...document.querySelectorAll('div[class*="block"], div[class*="Block"], li[class*="answer"], li[class*="Answer"]')]
          .filter(el => {
        ...
        if (candidates.length >= 2) {
          ansEls  = candidates;
          options = candidates.map(e=>(e.innerText||'').trim().split('\n')[0]).filter(Boolean);
        }
      } catch {}
    }

    /* sobe para o botão clicável */
    const btns = ansEls.map(e => {
      let x = e;
      for (let i=0; i<6; i++) {
        if (!x) break;
        const tag = x.tagName;
        const role = x.getAttribute?.('role');
        if (tag==='BUTTON' || tag==='A' || role==='button') return x;
        x = x.parentElement;
      }
      return e;
    });

    /* ── INPUTS de texto (dissertativa) ── */
    ...
    if (opts.length >= 2) {
      ...
      try {
        ...
        if (idx >= 0 && idx < els.length) {
          ...
          setUI('done', opts[idx]);
          setQ(s.question);
          try { window.ClawStats?.addKahoot(); window.ClawStats?.addHistory({type:'kahoot',question:s.question,answer:opts[idx]}); } catch{}
          await sleep(9000);
          clearHL(); busy=false; setUI('idle','Monitorando...'); setQ(''); return;
        }
      } catch(e) { stopBar(); console.error('[Claw]',e); }
      setUI('error','Não identificou — tente novamente');
      await sleep(3000);

    } else if (fresh.inputs.length > 0 || s.inputs.length > 0) {
      setUI('thinking','Respondendo dissertativa...');
      startBar();
      try {
  ...
  /* ================================================================
     GROQ — único processo, prompt de alta precisão
  ================================================================ */
  const SYS = `Você é um professor especialista em TODAS as matérias escolares brasileiras:
MATEMÁTICA: aritmética, álgebra, geometria, trigonometria, equações, porcentagem, frações, regra de três, juros, estatística, probabilidade, PA, PG, funções, matrizes, logaritmos
FÍSICA: cinemática, dinâmica, energia, eletricidade, magnetismo, óptica, termodinâmica, ondas
QUÍMICA: tabela periódica, ligações, reações, balanceamento, estequiometria, soluções, orgânica
BIOLOGIA: célula, genética, evolução, ecologia, anatomia, DNA/RNA
HISTÓRIA: Brasil e mundo, datas, guerras, revoluções, personagens
GEOGRAFIA: países, capitais, relevo, clima, biomas, geopolítica
PORTUGUÊS: gramática, ortografia, interpretação, literatura, figuras de linguagem
INGLÊS: gramática, vocabulário, interpretação
FILOSOFIA, SOCIOLOGIA, ATUALIDADES, CULTURA GERAL

Você nunca erra. Pense com cuidado e responda SOMENTE com o número (1, 2, 3 ou 4).`;

  async function askGroq(question, options) {
    const list = options.map((o,i) => `${i+1}. ${o}`).join('\n');
    const prompt = `QUESTÃO: ${question}\n\nALTERNATIVAS:\n${list}\n\nQual o número da alternativa correta? Responda APENAS com o número.`;
    ...
    for (let i=0; i<options.length; i++) {
      if (low.includes(options[i].toLowerCase().slice(0,12))) return i;
    }
    return -1;
  }
  ...
  async function askText(question) {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body   : JSON.stringify({
        model    : MODEL,
        messages : [
          { role:'system', content: SYS + '\nResponda dissertativas em até 5 palavras, direto ao ponto.' },
  ...
  function hlAnswer(els, idx) {
    ...
    els.forEach((el,i) => {
      if (i===idx) {
        ...
        el.style.setProperty('opacity',    '0.11',                           'important');
        el.style.setProperty('filter',     'grayscale(1) brightness(0.25)',  'important');
  ...
  function stopBar() {
    ...
    if(b) { b.style.width='100%'; setTimeout(()=>{if(b)b.style.width='0%';},600); }
  }
...
#__claw_k__{position:fixed!important;top:14px!important;right:14px!important;z-index:2147483647!important;width:258px;font-family:'Inter',system-ui,sans-serif;-webkit-font-smoothing:antialiased;pointer-events:auto;}
@keyframes _ckIn{from{opacity:0;transform:translateY(-8px) scale(.97)}to{opacity:1;transform:none}}
@keyframes _ckPulse{0%,100%{opacity:1}50%{opacity:.35}}
...
#__ck_ttl__{font-size:12px;font-weight:600;color:#f0efed;flex:1;letter-spacing:.1px;}
#__ck_bdg__{display:flex;align-items:center;gap:4px;font-size:9px;font-weight:500;color:#6b6b6b;background:#2a2a2d;border:1px solid #3a3a3e;border-radius:5px;padding:2px 7px;}
...
#__ck_dot__.t{background:#d4a85a;box-shadow:0 0 0 3px rgba(212,168,90,.2);animation:_ckPulse .9s ease-in-out infinite;}
